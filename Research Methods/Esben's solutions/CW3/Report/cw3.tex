...
\subsection{(p, k)-means}
    ...
    Again, $\mathbf{x}^n$ denotes that each component of $\mathbf{x}$ is raised to the $n$th power. The argument of convergence for this algorithm is similar to the argument for the standard $k$-means algorithm. That is, the objective decreases in step 1 and 2 and the objective is bounded below. Hence the algorithm converges. 
